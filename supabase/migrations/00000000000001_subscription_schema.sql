...
create policy "Can view own user data." on pos.users for select using (auth.uid() = id);
create policy "Can update own user data." on pos.users for update using (auth.uid() = id);
...
returns trigger as $$
begin
  insert into pos.users (id, full_name, avatar_url)
...
create policy "Allow public read-only access." on pos.products for select using (true);
...
create table pos.prices (
  ...
  product_id text references pos.products, 
...
create publication supabase_realtime for table pos.products, pos.prices;